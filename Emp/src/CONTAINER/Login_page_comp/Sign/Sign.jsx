...
const Sign = ({ set_login }) => {
    ...
    const forgot_password_fn = () => {
        
        const user_email = forgot_password_ref.current.value

        if (user_email && user_email.trim() !== '' && isValidEmail(user_email)) {

            send_forgotPassword_email(forgotPassword, user_email, dispatch, set_forgot_password, toast, set_React_loader)

        }
        else { toast.error(<div className="error_box">Enter Email</div>) }
    }
